...
alter table public.wholesalers
    add column pg_provider           text,
    add column pg_client_key         text,
    add column pg_secret_key_encrypted text;
...
alter table public.wholesaler_retailers
    add column allowed_payment_methods text[] not null default array['prepaid'];
...
alter table public.orders
    add column payment_status text check (payment_status in ('unpaid', 'paid', 'refunded')),
    add column pg_payment_key text,
    add column pg_order_id   text;
...
create table public.pg_pending_payments (
...
create index idx_pg_pending_payments_wholesaler on public.pg_pending_payments(wholesaler_id);
create index idx_pg_pending_payments_retailer on public.pg_pending_payments(retailer_id);
...
-- 승인 성공/실패 후 정리(삭제)는 서버(success/fail 라우트)가 buyer 세션으로 수행한다.
