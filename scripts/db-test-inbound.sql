...
insert into results (who,what,expected,result) values
 ...
 ('A사장','  └ 자동생성 → 새 상품 "채끝 1+"(소는 상품명 = 부위 등급, 축종은 화면 태그 — 113), 가격 필요, 판매중지 상태','true|채끝 1+|true|true', pg_temp.val($q$select (r->>'created')||'|'||(r->>'product_name')||'|'||(r->>'needs_price')||'|'||(r->>'needs_activation') from public.autocreate_product_for_scan((select id from ids where key='S5')) r$q$)),
