...
4. `/admin/admins` UI 구축 (`requireAdminGranter()` → `can_current_user_grant_admin()` RPC로 게이팅). **`is_verified = true` 계정은 승격 후보 목록에서 반드시 필터링/플래그 처리** — 안 하면 `PLATFORM_ADMIN_TARGET_ALREADY_VERIFIED` 에러로 실패함.
