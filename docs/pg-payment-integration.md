...
## 점검 3 — 유령주문·중복 처리 멱등성 (2026-09-24, `20260930000101_pg_order_idempotency.sql`)

토스 웹훅 수신 경로는 없고, 결제 확인은 "승인 콜백 + 재대조(주문내역 재방문·매일 크론)" 두 축이다. 유령주문 방지(승인 후에만 주문 생성, 저장된 기대금액과 대조)는 지켜지고 있었고, 아래를 고쳤다.

| # | 문제 | 처리 |
|---|---|---|
| 1 | `orders.pg_order_id`에 유니크가 없어, 콜백이 주문을 만든 직후 대기 행 삭제만 실패하면 다음 날 크론이 같은 결제로 두 번째 주문(알림톡도 두 번) | `orders(pg_order_id)` 부분 유니크 인덱스. `finalizePaidOrder`는 생성 전에 같은 `pg_order_id` 주문이 있으면 만들지 않고 대기 행만 정리(멱등), 유니크 위반도 같은 처리 |
| 2 | 핫딜 매진이면 주문을 삭제하는데 PG는 이미 승인 뒤라 "결제 완료, 주문 없음". 재대조는 매일 다시 실패 | **즉시 자동 환불(A안, 사장님 확정)** 후 "매진돼 환불 처리됨" 안내. 환불까지 실패하면 대기 행을 남겨 다음에 재시도 |
| 3 | 취소 승인이 "환불 → 상태 변경" 순서인데 상태 변경이 0건(그 사이 변경)이면 환불만 되고 `payment_status`는 `paid` | 환불이 나갔으면 상태 변경과 무관하게 `payment_status='refunded'` 기록 + 별도 안내 |
| 4 | 재대조가 토스가 돌려준 금액을 그대로 주문 총액으로 저장 | 대기 행 기대금액과 대조, 다르면 주문을 만들지 않고 기록만 |
| 5 | 실패 콜백(`fail/route.ts`)이 대기 행을 삭제 — 결제가 잡힌 뒤 실패 URL이 오면 복구 근거 소실 | 삭제 제거. 대기 행은 30분 뒤 재대조가 토스 상태를 보고 정리 |
| 6 | 크론 3개가 `CRON_SECRET` 미설정이면 인증 없이 실행 | 미설정이면 거부. **Vercel 환경변수에 `CRON_SECRET`이 있어야 크론이 돈다** |

미처리(낮음): 토스 쪽에서 취소된 결제(대시보드 취소·지급 거절)는 우리 `payment_status`가 `paid`로 남는다 — 웹훅 수신 경로가 없어서 생기는 한계. 계정 발급 후 웹훅 추가 시 함께.

검증: `scripts/db-test-pg-idempotency.sql`(유니크 위반·직접정산 NULL 허용), `classifyCreateOrderError` 단위테스트 3건, tsc. **실호출은 여전히 불가(토스 계정 미발급).**
