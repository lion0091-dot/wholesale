...
/**
 * 공급처 전표 입력 양식(.xlsx) — 종이·팩스로 받은 전표를 사무실에서 옮겨 적는 빈 엑셀.
 *
 * 첫 시트 "전표"는 헤더 한 줄뿐이다(예시 줄을 넣으면 그대로 올라가 진짜 줄이 되므로 예시는 안내 시트에 둔다).
...
export async function buildStatementTemplate(): Promise<Buffer> {
  ...
  const guide = [
    ...
    ["1. '전표' 시트에 종이 전표의 줄을 한 줄씩 옮겨 적습니다. 첫 줄(헤더)은 지우지 마세요."],
    ...
    ["5. 묶음번호 칸은 이력번호와 묶음번호가 둘 다 적힌 전표에서만 씁니다. 묶음번호만 있으면 이력번호 칸에 적으세요."],
