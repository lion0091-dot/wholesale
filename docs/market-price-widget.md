...
### 남은 과제
- **API 신청(data.go.kr, 자동승인) → `.env.local` + Vercel에 `KAPE_MARKET_PRICE_API_KEY` 등록 — 사용자가 직접.** 이거 하나만 하면 다음 크론 실행(매일 13시 KST)부터 실데이터가 채워짐.
- 키 발급 후 실제 XML 응답으로 재검증 필요: `CTotAmt` 인접 필드(전국 두수)의 정확한 영문 필드명(현재 `unit_count`는 항상 null), `startYmd`/`endYmd` 생략 시 기본 동작, 파싱 트리 재귀탐색 방식이 실제 응답 구조와 잘 맞는지.
- 상품 등록/수정 화면 실계정 라이브 검증(위젯이 실제로 뜨는지) — 아직 안 함, 다른 기능들과 동일하게 사용자가 직접 확인 필요.
