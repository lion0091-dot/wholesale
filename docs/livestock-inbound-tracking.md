...
### 상품 목록 신선도 표시 + 경락가 열람 범위 (4단계)
- `master_livestock.packing_date`(포장처리일자) 추가. 20260930000053에서 빠져 있었다. 인증키 미발급이라 실응답에 실제로 오는지는 미확인 — 안 오면 NULL로 남을 뿐이다.
- 상품 하나는 박스 여러 개로 채워져 있어 도축일이 하나가 아니다. 목록에는 **가장 오래된 박스 기준**으로 보여준다 — 선입선출 관리에서 실제로 봐야 하는 값이 "가장 오래 묵은 재고가 며칠 됐나"이기 때문이다. 14일/30일에서 색이 바뀐다. 박스별 상세는 `get_product_stock_boxes()`.
- 목록에서 상품마다 따로 조회하면 N+1이라 `get_product_stock_summary()`로 한 번에 집계해 받는다.
- **경락가(`market_price_snapshots`) 열람을 공급사로 좁혔다.** 기존 정책은 로그인 사용자 전원에게 열려 있어 카카오로 로그인한 바이어(식당)도 조회할 수 있었다. 공공 시세는 공급사의 매입가 판단용이라 고객에게 노출되면 안 된다. 위젯 자체는 원래 상품 등록 화면에만 있고 미니샵에는 없었지만, DB 권한까지 막아야 확실하다.
