...
export interface OrderJoinRow {
  ...
  ordered_at: string;
  order_items: Pick<OrderItem, "product_name" | "quantity" | "category">[] | null;
  retailers: { restaurant_name: string } | { restaurant_name: string }[] | null;
}

export const ORDER_LIST_SELECT_COLUMNS =
  "id, order_number, status, total_amount, delivery_address, ordered_at, order_items ( product_name, quantity, category ), retailers ( restaurant_name )";
...
export const ORDER_LIST_SELECT_COLUMNS_RETAILER_INNER =
  "id, order_number, status, total_amount, delivery_address, ordered_at, order_items ( product_name, quantity, category ), retailers!inner ( restaurant_name )";
...
/** "소 한우 1++ 등심 2 외 2건" 형태의 품목 요약 */
export function summarizeItems(
  items: Pick<OrderItem, "product_name" | "quantity" | "category">[]
): string {
  if (items.length === 0) {
    return "품목 정보 없음";
  }
  ...
  const [first] = items;
  const head = `${composeProductDisplayName(first.category, first.product_name)} ${Number(first.quantity)}`;
