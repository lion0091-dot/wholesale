...
- 과금 대상은 `wholesaler_retailers.status = 'active'`(거래중)인 거래처만 — 거래중지(blocked)는 카운트에서 제외된다.
- 거래처 1곳당 월 **5,000원**. `lib/supplier/billing.ts`의 `MONTHLY_FEE_PER_RETAILER` 상수.
...
- `/billing-locked`는 사유(연체/해지/체험만료)와 "이번 달 구독료"(거래중 거래처 수 × 5,000원)를 보여주고, 결제는 관리자에게 문의하라고 안내한다.
...
- `app/admin/suppliers/page.tsx` + `supplier-approval-list.tsx` — 공급사별 "이번 달 구독료"(거래중 거래처 수 × 5,000원)와 체험 잔여일수/차단 여부를 목록에 표시. 거래처 수는 `wholesaler_retailers`를 `active`로 필터링해 wholesaler_id별로 집계.
