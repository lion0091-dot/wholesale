...
CREATE TABLE IF NOT EXISTS public.inbound_document_lines (
    ...
    -- 원산지(국내산/미국산 등). 필수 항목이다 — 원산지 허위표시로 이어질 수
    -- 있어서다(사장님 확정 2026-09-23). 알 수 있는 경로가 셋이라(서류 기재 /
    -- 이력번호 공공조회 / 연결된 상품의 products.origin) 셋 다 막혔을 때만
    -- 미흡으로 본다. 서류와 연결 상품의 원산지가 다르면 상품을 잘못 고른 것이다.
    origin       TEXT,
