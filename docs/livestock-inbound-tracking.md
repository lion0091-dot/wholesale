...
### 상품관리 화면: 경락가 노출, 이력 정보는 비노출 (4단계)
- **상품 목록 조회는 기존 방식 그대로 둔다.** 한때 도축일/포장일을 목록에 집계해 붙였다가 되돌렸다 — 공급사가 상품관리에서 볼 정보가 아니라는 판단이다(잠긴 결정). 이력 정보는 입고/재고 화면 쪽에서 다룬다.
- `master_livestock.packing_date`(포장처리일자)는 컬럼과 조회 함수만 남겨둔다. 20260930000053에서 빠져 있어 추가했고, 인증키 미발급이라 실응답에 오는지는 미확인 — 안 오면 NULL로 남을 뿐이다.
- `get_product_stock_summary()` / `get_product_stock_boxes()`는 DB에 남겨둔다. 상품 목록에서는 더 이상 호출하지 않지만 입고·재고 화면에서 쓸 값이다.
...
1. 도축일이 다른 박스 2개 → `box_count 2`, 가장 오래된 도축일/최근 도축일/가장 오래된 포장일, 등급 `1+, 1++` 집계 (함수는 남겨둠, 상품 목록에서는 미사용)
2. 공급사 계정 → 경락가 조회됨
3. 바이어(식당) 계정 → 경락가 **0건**
