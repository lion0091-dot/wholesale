...
- 필터: 기간(기본 최근 30일)·상품·유형. **URL 쿼리로 유지**해서 새로고침·뒤로가기에도 조건이 살아 있고 링크로 공유된다.
...
**검증 (5종 통과)**: 입고 2건 + 손실 1건 + 출고 1건이 오래된 순으로 시점 재고(8.20 → 15.70 → 15.40 → 10.40)와 함께 조회됨 / 기간 요약(입고 15.70, 출고 -5.00, 손실 -0.30) / **출고만 필터해도 시점 재고가 전체 기준 10.40 유지** / 마지막 시점 재고가 `products.stock_quantity`와 일치 / 다른 업체 계정은 0건.
